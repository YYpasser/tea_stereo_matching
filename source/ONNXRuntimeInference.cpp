...
class stereo::ONNXRuntimeInference::ONNXRuntimeImpl
{
public:
	ONNXRuntimeImpl();
	~ONNXRuntimeImpl();
...
stereo::ONNXRuntimeInference::ONNXRuntimeInference()
{
	this->impl = std::make_unique<ONNXRuntimeImpl>();
}
...
void stereo::ONNXRuntimeInference::loadModel(const std::string& modelPath)
{
...
void stereo::ONNXRuntimeInference::compute(const cv::Mat& leftImage, const cv::Mat& rightImage, cv::Mat& disparity)
{
	...
	auto end = std::chrono::steady_clock::now();
	auto tt = std::chrono::duration_cast<std::chrono::microseconds>(end - start);	
	disparity = this->impl->padder.unpad(disp);
	LOG_INFO("Disparity map computed. Timing: " + utils::formatMilliseconds(tt.count() / 1000.0) + " ms.");
}

void stereo::ONNXRuntimeInference::compute(const std::vector<cv::Mat>& leftImages, const std::vector<cv::Mat>& rightImages, std::vector<cv::Mat>& disparities)
{
	...
	// Tensor[N, C, H, W] -> Mat[N, H, W, C] & unpadding
	float* floatarr = dispTensors[0].GetTensorMutableData<float>();
	...
	for (int i = 0; i < numImages; ++i)
	{
		...
		disparities.push_back(std::move(this->impl->padder.unpad(disp)));
	}
}

stereo::ONNXRuntimeInference::ONNXRuntimeImpl::ONNXRuntimeImpl()
{
}
