...
public:
    ...
    std::vector<stereo::StereoPair<GoodChessboardImage>> m_goodImagesList;
    std::vector<cv::Vec6d> m_xyzrpy;
    stereo::CameraIntrinsic m_stereoIntrinsic; /*!< 双目内参 */
    stereo::EpipolarRectifyMap m_rectifyMap;   /*!< 双目极线校正映射 */
...
void calib::StereoHandEyeAXXB::calibrateEyeInHand(const bool& showCorners, const bool& writeCorners)
{
    ...
    try
    {
        ...
        this->impl->m_stereoIntrinsic = stereo::CameraIntrinsic(K, D);
        this->impl->m_rectifyMap = stereo::EpipolarRectifyMap(stereoParams.map);
    }
