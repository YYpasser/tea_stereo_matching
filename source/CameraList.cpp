...
{
    std::cout << "Found " << this->list.size() << " camera devices." << std::endl;
    std::cout << "Camera device list:" << std::endl;
    for (auto p = this->list.begin(); p != this->list.end(); ++p)
    {
        auto i = std::distance(this->list.begin(), p);
        std::cout << "[" << i + 1 << "] \"" << p->friendlyName << "\"" << std::endl;
        std::cout << "     device ID = " << i << std::endl;
        std::cout << "     Symbolic link: " << p->symbolicLink << std::endl;
        std::cout << "     Device Location: " << p->portLocation << std::endl;
        std::cout << "     Support resolution with fps:  " << std::endl;
        for (auto it = p->prop.begin(); it != p->prop.end(); ++it)
        {
            auto [res, fps, enc] = *it;
            std::cout << "        " << res->width << " x " <<
                res->height << " with " << *fps << " fps." << std::endl;
        }
    }
}
